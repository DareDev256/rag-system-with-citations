...
![Tests](https://img.shields.io/badge/tests-429_passing-2ea44f?style=flat-square)
...
- **18-layer security** — rate limiting (thread-safe), API auth, input validation, output sanitization, HSTS, CSP (validated), request tracing, SSRF prevention (base URL + embeddings), webhook HMAC verification, header injection defense
...
- **429 tests, zero external deps** — full mock coverage, runs without API keys or FAISS indexes
